...
//	set_PID_params(&PIDx, 1100, -1100, 0.0022, 0.047, 0.012); //po zmianie na kod bez Ts
	...
	//final:
	set_PID_params(&PIDx, 1100, -1100, 0.02, 0.044, 0.02); //po zmianie na kod bez Ts
	set_PID_params(&PIDy, 1100, -1100, 0.02, 0.044, 0.02); //po zmianie na kod bez Ts

	// do wykresow:
	// step1 - Samo Kp
	set_PID_params(&PIDx, 1100, -1100, 0.021, 0, 0);
	// step2 - Kp i Kd
	set_PID_params(&PIDx, 1100, -1100, 0.021, 0, 0.023);
	// step3 - dodac Ki
	set_PID_params(&PIDx, 1100, -1100, 0.021, 0.4, 0.023);

	// drugie podejscie
//	set_PID_params(&PIDx, 1100, -1100, 0.78, 0, 0);
	set_PID_params(&PIDx, 1100, -1100, 0.65, 0, 0);
//	set_PID_params(&PIDx, 1100, -1100, 0.34, 0, 0);

	set_PID_params(&PIDx, 1100, -1100, 0.38, 0.07, 0.29); // wykres
	set_PID_params(&PIDx, 1100, -1100, 0.05, 0.3, 0.22); // 1.0 sek do ustalenia - faworyt
//	set_PID_params(&PIDx, 1100, -1100, 0.04, 0.46, 0.24); //1.4  sek do ustalenia
//	set_PID_params(&PIDx, 1100, -1100, 0.04, 0.52, 0.19); //1.4  sek do ustalenia
//	set_PID_params(&PIDx, 1100, -1100, 0.057, 0.33, 0.22); // 1.0 sek do ustalenia
	set_PID_params(&PIDx, 1100, -1100, 0.06, 0.3, 0.24); // 0.8 sek -fave nr 2
	set_PID_params(&PIDx, 1100, -1100, 0.08, 0.3, 0.24); // 0.7 sek - trzeba zwiekszac Kp, lub Kd?
	set_PID_params(&PIDx, 1100, -1100, 0.08, 0.3, 0.27); // 1 sek - za duze Kd
	set_PID_params(&PIDx, 1100, -1100, 0.08, 0.3, 0.23); // 0.8 sek, ale troche oscylacji ze wzgledu na kd
	set_PID_params(&PIDx, 1100, -1100, 0.08, 0.3, 0.12); // 0.8 sek, bo kd mniej przeszkadza
	set_PID_params(&PIDx, 1100, -1100, 0.08, 0.3, 0); // tez 0.8sek - poszukac KD nie dajacego oscylacji
	set_PID_params(&PIDx, 1100, -1100, 0.08, 0.3, 0.07); // 0.75s Kd male oscylacje


	set_PID_params(&PIDx, 1100, -1100, 0.31, 0, 0.17);
	set_PID_params(&PIDx, 1100, -1100, 0.26, 0.36, 0.12);
	set_PID_params(&PIDx, 1100, -1100, 0.28, 0.38, 0.12);
	set_PID_params(&PIDx, 1100, -1100, 0.183, 0.312, 0.184);
...
//				PIDx.e_in = ptr_env->PlatformX - ptr_env->X_Real; // joystick - zmierzone
				...
				PIDx.e_in = 66 - PIDx.y_out; // TESTOWANIE WYKRESOW PID
				...
				ptr_env->next_angle_Pitch = -1
						* get_angle_from_PID_output(&PIDx, 35, 1); // osia X steruje Pitch   5.3   0,169721656 bylo 9.71 a potem 9.48, a potem 35

				ptr_env->next_angle_Roll = get_angle_from_PID_output(&PIDy, 35,
						1); // osia Y steruje Roll 0.45   0,169721656

				//zrob ruch - najlepiej ze struktury, ktora sie updatuje w przerwaniach
